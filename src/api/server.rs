...
use super::state::{ApiState, HealthResponse, ResultsResponse, StatusResponse, ExecutionStatus};
...
async fn results_handler(
...
) -> (StatusCode, Json<ResultsResponse>) {
    let status = state.status();
    ...
    (
        status_code,
        Json(ResultsResponse { status, results }),
    )
}
